...
public class VoltageStrategy implements RuleStrategy{

    @Override
    public WarnVO ruleAlert(String batteryType, Map<String, BigDecimal> data, Rule rule) {
        BigDecimal maxVoltage = data.getOrDefault("Mx", BigDecimal.ZERO);
        BigDecimal minVoltage = data.getOrDefault("Mi", BigDecimal.ZERO);
